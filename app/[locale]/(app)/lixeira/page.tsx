        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex items-center gap-3 text-xs text-muted-foreground">
            <span className="flex items-center gap-1">
              ...
              {t('deletedAt')} {format(new Date(query.deletedAt!), 'd MMM', { locale })}
            </span>
          </div>
          {query.description && (
      ...
      <CardContent className="flex items-center gap-4 p-4">
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex items-center gap-3 text-xs text-muted-foreground">
            <span className="flex items-center gap-1">
              <Clock className="h-3 w-3" />
              {t('deletedAt')} {format(new Date(routine.deletedAt!), 'd MMM', { locale })}
            </span>
          </div>
          {routine.description && (
            <p className="mt-1 truncate text-sm text-muted-foreground">
  ...
  return (
    <>
          ...
          <Tabs defaultValue="queries" value={activeTab} onValueChange={setActiveTab}>
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                <TabsList>
                  <TabsTrigger value="queries">{t('queriesTab')}</TabsTrigger>
                  <TabsTrigger value="routines">{t('routinesTab')}</TabsTrigger>
                </TabsList>
              ...
              {currentCount > 0 && (
                ...
                >
                  ...
                  {activeTab === 'queries' ? t('emptyQueries') : t('emptyRoutines')}
                </Button>
              )}
            </div>
              ...
              {trashedQueries.length > 0 ? (
                ...
                <Empty className="border">
                  <EmptyHeader>
                    ...
                    <EmptyTitle>{t('queriesTrashEmpty')}</EmptyTitle>
                    <EmptyDescription>
                      {t('queriesEmptyDescription')}
                    </EmptyDescription>
                  </EmptyHeader>
                </Empty>
              )}
              ...
              {trashedRoutines.length > 0 ? (
                ...
                <Empty className="border">
                  <EmptyHeader>
                    ...
                    <EmptyTitle>{t('routinesTrashEmpty')}</EmptyTitle>
                    <EmptyDescription>
                      {t('routinesEmptyDescription')}
                    </EmptyDescription>
                  </EmptyHeader>
                </Empty>
              )}
      ...
      <DeleteConfirmModal
        ...
        description={t('permanentDeleteDescription', { title: deletingItem?.title ?? '' })}
      ...
      <DeleteConfirmModal
        ...
        title={activeTab === 'queries' ? t('emptyQueriesTrashTitle') : t('emptyRoutinesTrashTitle')}
        description={activeTab === 'queries'
          ? t('emptyQueriesTrashDescription', { count: trashedQueries.length })
          : t('emptyRoutinesTrashDescription', { count: trashedRoutines.length })
        }
        confirmText={t('emptyConfirm')}
        isLoading={queryStore.isSubmitting || routineStore.isSubmitting}
