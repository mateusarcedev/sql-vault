...
export default function QueryDetailPage({ params }: QueryDetailPageProps) {
  ...
  if (!query) {
    return (
      <>
        <AppHeader title={t('queryNotFound')} showSearch={false} showNewButton={false} />
        <main className="flex-1 overflow-auto p-6">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-2xl font-bold">{t('queryNotFound')}</h2>
            <p className="mt-2 text-muted-foreground">
              {t('queryNotFoundDescription')}
            </p>
            <Button asChild className="mt-4">
              <Link href="/consultas">{t('backToQueries')}</Link>
            </Button>
          </div>
        </main>
      </>
  ...
  return (
    <>
        ...
        <div className="mx-auto max-w-4xl space-y-6">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                />
                {query.isFavorite ? t('favorited') : t('favorite')}
              </Button>
              <Button
                variant="outline"
                size="sm"
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
            <span className="flex items-center gap-1">
              ...
              {t('createdAt')} {format(new Date(query.createdAt), 'PPP', { locale })}
            </span>
            <span className="flex items-center gap-1">
              <Copy className="h-4 w-4" />
              {query.copyCount} {tCommon('copies')}
            </span>
            <span className="flex items-center gap-1">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-3">
              <div>
                <CardTitle className="text-lg">{t('sqlCode')}</CardTitle>
                <CardDescription>{t('currentVersion')}</CardDescription>
              </div>
      ...
      <DeleteConfirmModal
        ...
        title={t('deleteQuery')}
        description={t('deleteDescription', { title: query.title })}
