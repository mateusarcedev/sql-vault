...
export default function RoutineDetailPage({ params }: RoutineDetailPageProps) {
  ...
  const tDetail = useTranslations('queryDetail')
  ...
  const handleCopy = async (sql: string) => {
    try {
      ...
      toast.success(tDetail('copiedToClipboard'))
    } catch {
      toast.error(tDetail('errorCopying'))
    }
  }
  ...
  const handleToggleFavorite = async () => {
    ...
    toast.success(
      routine.isFavorite
        ? tDetail('removedFromFavorites')
        : tDetail('addedToFavorites')
    )
  }
  ...
  const handleCompare = (versionA: any, versionB: any) => {
    setDiffModal({ open: true, versionA, versionB })
  }

  if (isLoading || !isInitialized) {
    return (
      <>
        <AppHeader title={tCommon('loading')} showSearch={false} showNewButton={false} />
  ...
  if (!routine) {
    return (
      <>
        <AppHeader title={t('routineNotFound')} showSearch={false} showNewButton={false} />
        <main className="flex-1 overflow-auto p-6">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-2xl font-bold">{t('routineNotFound')}</h2>
            <p className="mt-2 text-muted-foreground">
              {t('routineNotFoundDescription')}
            </p>
  ...
  return (
    <>
        ...
        <div className="mx-auto max-w-4xl space-y-6">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                />
                {routine.isFavorite ? tDetail('favorited') : tDetail('favorite')}
              </Button>
              <Button
                variant="outline"
                size="sm"
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
            <span className="flex items-center gap-1">
              ...
              {tDetail('createdAt')} {format(new Date(routine.createdAt), 'PPP', { locale })}
            </span>
            <span className="flex items-center gap-1">
              <Copy className="h-4 w-4" />
              {routine.copyCount} {tCommon('copies')}
            </span>
            <span className="flex items-center gap-1">
              <History className="h-4 w-4" />
              {tDetail('versions', { count: routine.versions.length })}
            </span>
            {routine.returnType && (
              <span className="flex items-center gap-1 ml-auto">
                <strong className="text-foreground">{tDetail('returnLabel')}</strong> {routine.returnType}
              </span>
            )}
          </div>
          ...
          {routine.parameters && routine.parameters.length > 0 && (
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-lg">{tDetail('parameters')}</CardTitle>
                <CardDescription>
                  {tDetail('parametersDescription', { type: routine.type })}
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b bg-muted/50">
                        <th className="p-3 text-left font-medium">{tDetail('paramName')}</th>
                        <th className="p-3 text-left font-medium">{tDetail('paramType')}</th>
                        {routine.type === 'procedure' && (
                          <th className="p-3 text-left font-medium">{tDetail('paramDirection')}</th>
                        )}
                      </tr>
                    </thead>
                    <tbody>
            ...
            <CardHeader className="flex flex-row items-center justify-between pb-3">
              <div>
                <CardTitle className="text-lg">
                  {tDetail('sqlCode')} ({routine.type.toUpperCase()})
                </CardTitle>
                <CardDescription>{tDetail('currentVersion')}</CardDescription>
              </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">{tDetail('versionHistory')}</CardTitle>
              <CardDescription>
      ...
      <DeleteConfirmModal
        ...
        title={t('deleteTitle')}
        description={t('deleteDescription', { name: routine.name })}
